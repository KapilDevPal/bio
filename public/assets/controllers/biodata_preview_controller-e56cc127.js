...
export default class extends Controller {
  ...
  connect() {
    ...
    // Debug: Test createExportHTML method
    setTimeout(() => {
      try {
        const testHTML = this.createExportHTML()
        console.log('Test createExportHTML result:', testHTML.substring(0, 300) + '...')
      } catch (error) {
        console.error('Error in createExportHTML:', error)
      }
    }, 1000)
  }
  ...
  async exportPDF(event) {
    ...
    try {
      ...
      // Get biodata and template
      const biodata = JSON.parse(this.biodataDataTarget.textContent)
      const template = this.selectedTemplateTarget.textContent.trim()
      
      // Create a simple export HTML with inline styles
      let exportHTML = this.createSimpleExportHTML(biodata, template)
  ...
  createSimpleExportHTML(biodata, template) {
    console.log('createSimpleExportHTML called with:', { biodata, template })
    
    // Create a very simple HTML structure with inline styles
    const photoHtml = biodata.photo ? 
      `<div style="text-align: center; margin-bottom: 24px;">
        <img src="${biodata.photo}" style="width: 128px; height: 128px; border-radius: 50%; margin: 0 auto; object-fit: cover; border: 4px solid #ffffff; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1);">
      </div>` : ''
    
    let html = `
      <div style="background-color: #ffffff; padding: 32px; max-width: 800px; margin: 0 auto; font-family: Arial, sans-serif; border: 1px solid #d1d5db;">
        ${photoHtml}
        <div style="text-align: center; margin-bottom: 32px; border-bottom: 2px solid #2563eb; padding-bottom: 24px;">
          <h1 style="font-size: 30px; font-weight: bold; color: #111827; margin-bottom: 8px;">Biodata</h1>
          <div style="width: 80px; height: 4px; background-color: #2563eb; margin: 0 auto;"></div>
    ...
    // Add sections
    const sections = [
      { title: 'Personal Information', data: biodata.personal },
      { title: 'Family Information', data: biodata.family },
      { title: 'Contact Information', data: biodata.contact },
      { title: 'Partner Preferences', data: biodata.preferences }
    ]
    ...
    sections.forEach(section => {
      if (section.data) {
        html += `
          <div style="margin-bottom: 32px;">
            <h2 style="font-size: 20px; font-weight: bold; color: #2563eb; margin-bottom: 16px; border-bottom: 1px solid #bfdbfe; padding-bottom: 8px;">${section.title}</h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
        `
